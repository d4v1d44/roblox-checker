...
async function tryRobloxLogin(email, password) {
    ...
    try {
        ...
        if (loginRes.data && loginRes.data.user) {
            ...
            if (err.code === 2) return { success: false, error: "Credenciais inválidas" };
            if (err.code === 0 && err.message.includes("TwoStepVerification")) return { success: false, error: "2FA necessário" };
